...
<aside class="app-sidebar">
    ...
    <ul class="side-menu app-sidebar3">
                        ...
                        @if(!empty($item['children']))
                            ...
                            <ul class="slide-menu">
                                @foreach($item['children'] as $child)
                                    <li>
                                        <a href="{{ $child['route'] ? route($child['route']) : $child['url'] }}" class="slide-item">@if (!is_null($child['icon'])&& app()->getLocale()!='fa') <i class="slide-child-icon {{$child['icon']}}"></i> @endif{{ __($child['label']) }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        @else
        ...
        <li class="slide">
            <a class="side-menu__item" href="{{ route('admin.panel-notifications.index') }}">
                <span class="side-menu__icon fa-solid fa-bell"></span>
                <span class="side-menu__label">{{ __('Panel notifications') }}</span>
            </a>
        </li>
        @endif
            ...
            @if (App\Services\HelperService::extensionSaaS())
                @if (config('payment.referral.enabled') == 'on')
                    <div class="side-plan-wrapper mt-4 text-center p-3 pl-5 pr-5">
                        ...
                        <span class="fs-12 mt-4" style="color: #b5bac1">{{ __('Invite your friends and get') }} {{ config('payment.referral.payment.commission') }}% @if (config('payment.referral.payment.policy') == 'all') {{ __('of all their purchases') }} @else {{ __('of their first purchase') }}@endif</span>
